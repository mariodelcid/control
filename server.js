    ...
    try {
          ...
          if (ingredients) {
                  for (const i of ingredients) {
                            await pool.query('INSERT INTO product_ingredients (product_id, name, qty, unit_cost) VALUES ($1,$2,$3,$4)',
                                                       [prodId, i.name, i.qty, i.unitCost]);
                  }
          }
          if (packaging) {
                  for (const p of packaging) {
                            await pool.query('INSERT INTO product_packaging (product_id, name, qty, unit_cost) VALUES ($1,$2,$3,$4)',
                                                       [prodId, p.name, p.qty, p.unitCost]);
                  }
          }
...
app.put('/api/products/:id', async (req, res) => {
    try {
          ...
          if (ingredients !== undefined) {
                  ...
                  for (const i of ingredients) {
                            await pool.query('INSERT INTO product_ingredients (product_id, name, qty, unit_cost) VALUES ($1,$2,$3,$4)',
                                                       [req.params.id, i.name, i.qty, i.unitCost]);
                  }
          }
          if (packaging !== undefined) {
                  ...
                  for (const p of packaging) {
                            await pool.query('INSERT INTO product_packaging (product_id, name, qty, unit_cost) VALUES ($1,$2,$3,$4)',
                                                       [req.params.id, p.name, p.qty, p.unitCost]);
                  }
          }
          res.json({ ok: true });
    } catch (err) {
...
app.put('/api/overhead', async (req, res) => {
    try {
          for (const [k, v] of Object.entries(req.body)) {
                  await pool.query('INSERT INTO overhead (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value=$2',
                                           [k, Number(v)]);
          }
          res.json({ ok: true });
    } catch (err) {
